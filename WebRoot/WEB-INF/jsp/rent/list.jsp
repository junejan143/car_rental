...
<html>
  ...
  <title>无标题文档</title>
    ...
    */
     ...
     *  租赁管理
     */
     function toRent(car_id){
            //成功需要注意jquery的版本必须是1.7+以上
            var d = top.dialog({
                width:700,
                height:400,
                title: '业务管理 | 租赁',
                url:'sys/toAddRentAction.action?car_id='+car_id,//可以是一个访问路径Action|Servlet等或者jsp页面资源
                onclose: function () {
                    if (this.returnValue) {
    ...
    function toRentUpdate(car_id){
            ...
            var d = top.dialog({
                ...
                height:400,
                title: '业务管理 | 预定',
                url:'sys/toUpdateRentAction.action?car_id='+car_id,//可以是一个访问路径Action|Servlet等或者jsp页面资源
                onclose: function () {
                    if (this.returnValue) {
                        if(this.returnValue=="success"){
                            window.location.reload();
...
<body>
    ...
    <li>汽车租赁信息</li>
        ...
        <div class="tools">
            <form action="sys/findRentAction.action" method="post" name="searchForm">
                <ul class="seachform">
                <li><label>车辆名称</label>
        ...
        <tbody>
          ...
          <s:iterator value="pageBean.datas" var="carInfo" status="st">
            <tr>
                ...
                <td>
                <a href="javascript:void(0)" onclick="toRent('<s:property value="#carInfo.car_id"/>')" class="tablelink">租赁</a>&nbsp;&nbsp;|
                <a href="javascript:void(0)" onclick="toRentUpdate('<s:property value="#carInfo.car_id"/>')" class="tablelink">预定</a>
                </td>
            </tr> 
         </s:iterator>
        </tbody>
